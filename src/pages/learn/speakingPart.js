...
const SpeakingSection = ({route}) => {
  ...
  const onStopRecord = async () => {
    try {
      ...
      // Unlink the temporary WAV file if it exists
      if (await RNFS.exists(wavFilePath)) {
        await RNFS.unlink(wavFilePath);
      }

      // Convert the recording to a desired format
      ...
      if (ReturnCode.isSuccess(returnCode)) {
        ...
        if (exists) {
          console.log(`WavefilePath: ${wavFilePath}`);
          dispatch(transcribeAudio(wavFilePath));
        } else {
          console.log(
            'Converted file does not exist, fallback to original path',
          );
          // If needed, handle the fallback to play the original recording path
        }
      } else {
  ...
  const RecordButton = () => (
    <TouchableOpacity
      ...
      style={{
        ...
        backgroundColor: isLoading ? '#ccc' : '#F08080',
        borderRadius: 50,
        width: 88,
        height: 88,
    ...
    >
      {isLoading ? (
        ...
        <Image source={mic_ico} style={{width: 50, height: 50}} />
      )}
      <Text style={styles.text_m}>Hold to speak</Text>
    </TouchableOpacity>
  );
