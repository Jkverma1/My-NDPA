...
const Overall_part_1 = require('../../../../assets/icons/learn/overall/overall_part_1.png');
const Overall_part_2 = require('../../../../assets/icons/learn/overall/overall_part_2.png');
const close_ico = require('../../../../assets/icons/m_close_ico.png');
const idea_ico = require('../../../../assets/icons/learn/idea_ico.png');

const { width: screenWidth } = Dimensions.get('window');

const OverallProcess = ({ navigation }) => {
  const route = useRoute();
  const { move, part } = route.params;
  const bgImage = part === 1 ? Overall_part_1 : Overall_part_2;
  const modalText = part === 1 ? "Imagine that you're embarking on a journey to prioritize your overall wellbeing." : "Let’s summarize all information about overall wellbeing"
  ...
  const handleStart = () => {
    setModalVisible(false);
    navigation.navigate(part === 1 ? "OverallSection" : 'PracticeOverallSection');
  };

  const handleClickMove = async () => {
    console.log('-------------data--------------');
    navigation.navigate('OverallProcessSection', { move: false , part: 2 });
  };
