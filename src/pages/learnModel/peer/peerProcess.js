...
const Peer_part_1 = require('../../../../assets/icons/learn/peer/Peer_part_1.png');
const Peer_part_2 = require('../../../../assets/icons/learn/peer/Peer_part_2.png');
const close_ico = require('../../../../assets/icons/m_close_ico.png');
const idea_ico = require('../../../../assets/icons/learn/idea_ico.png');

const { width: screenWidth } = Dimensions.get('window');

const PeerProcess = ({ navigation }) => {
  const route = useRoute();
  const { move, part } = route.params;
  const bgImage = part === 1 ? Peer_part_1 : Peer_part_2;
  const modalText = part === 1 ? "Imagine that your friend is experiencing peer difficulties. Let’s learn information to help him." : "Let’s summarize all information about peer difficulties"
  ...
  const handleStart = () => {
    setModalVisible(false);
    navigation.navigate(part === 1 ? "PeerSection" : 'PracticePeerSection');
  };

  const handleClickMove = async () => {
    console.log('-------------data--------------');
    navigation.navigate('PeerProcessSection', { move: false , part: 2 });
  };
